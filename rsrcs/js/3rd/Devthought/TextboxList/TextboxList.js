...
var TextboxList = new Class({
  ...
  clearList : function() {
    ...
    var editableElement = this.list.getLast();
    editableElement.getChildren('input').set('value','');  // last element reset
    lastElement = $(editableElement).getPrevious(); // skip last item (editable)
    if (!lastElement) return;

    do {
      priorElement = $(lastElement).getPrevious();
      deleteme.push(this.getBit(lastElement));
      lastElement = priorElement;
    } while(lastElement);

    // clean
    for(var i=0;i<deleteme.length;i++) deleteme[i].remove();
