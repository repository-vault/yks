...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/1999/xhtml">
  ...
  <xsl:template match="ks_form">
    <form class="ks_form jsx_form {@class}" method="post" action="{//jsx/@href_ks}">
  ...
  <xsl:when test="$item_type and contains(',string,int,time,sha1,hidden,',concat(',',$item_type,','))">
    ...
    <xsl:if test="$item_type='sha1'"><xsl:attribute name="type">password</xsl:attribute></xsl:if>
    <xsl:copy-of select="@value|@style|@name|@id"/>
    </input>
  </xsl:when>

  <xsl:when test="$item_type='file'">
    <input type="file" name="{$name}"><xsl:copy-of select="@style"/></input>
  </xsl:when>
  ...
  <xsl:when test="$type='enum'">
    <xsl:variable name="value" select="@value"/>
      ...
      <xsl:attribute name='name'><xsl:value-of select='$name'/><xsl:if test="string($mode)='checkbox'">[]</xsl:if></xsl:attribute>

      <xsl:if test="contains(@value,$val) or $value=$val"><xsl:attribute name="checked">checked</xsl:attribute></xsl:if>
            </input> <label for="{$id}"> <xsl:value-of select="$val"/></label></div>
          </xsl:for-each>
          </div>
      </xsl:when>
