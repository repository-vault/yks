...
class Crypt_RSA_KeyPair
{
    ...
    function toOpenShhAuthorizedFile(){
      //Crypt_RSA_KeyPair::_ASN1Store
      $algo = "ssh-rsa";
      $str = "";
      $str .= $algo." ";

      $tmp = "";
      $tmp .= Crypt_RSA_KeyPair::WriteString($algo);
      $tmp .= Crypt_RSA_KeyPair::WriteString($this->getPublicKey()-> getExponent()) ;
      $tmp .= Crypt_RSA_KeyPair::WriteString(strrev($this->getPublicKey()-> getModulus())) ;
      //$tmp =  Crypt_RSA_KeyPair::_ASN1Store($tmp, 0);
      return $tmp;
      return $str; 
    }
    /**
