...
function imageglow($img_back, $img_mask, $color){
    ...
    for($x=0;$x<$img_w;$x++){
        for($y=0;$y<$img_h;$y++){
            $value=imgs::colorgray(imgs::colordec(imagecolorat($img_glow,$x,$y)));
...
function imageboldedge($img_mask,$border){
    $img_border = imgs::imageduplicate($img_mask); $color_base = imgs::colorgray($border);
    imagefilter ($img_border,IMG_FILTER_EDGEDETECT);
    $pic_w=imagesx($img_mask);$pic_h=imagesy($img_mask);
    $dest = imgs::imagecreatetruealpha($pic_w, $pic_h);
    for($x=0;$x<$pic_w;$x++){
        for($y=0;$y<$pic_h;$y++){
            $current = imgs::colorgray(imgs::colordec(imagecolorat($img_border,$x,$y)));
            $base = imgs::colordec(imagecolorat($img_mask,$x,$y));
            if($current == imgs::GRAY_LVL && $base['alpha']==TRANSPARENT_LVL) continue;
            $weight = abs($current-imgs::GRAY_LVL)/imgs::GRAY_LVL;
            if($base['alpha']!=0) $weight = ($weight+1)/2;
            $border['alpha'] = (1-$weight)*127; $to = imgs::colorget($border);
...
function imagefilldegrad($dest, $mask, $color_base, $colors_interval){
    ...
    list($from, $to) = $colors_interval;
    $from = imgs::colordec($from); $to = imgs::colordec($to);

    $dest  = imgs::imageduplicate($dest);
