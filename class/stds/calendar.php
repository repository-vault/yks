<?php
  class calendar {
    ...
    function __construct($authentification_type = 'oauth2.0_application') {
      ...
      if($this->authentification_type == 'public_server')
        $this->client->setDeveloperKey($this->key);

      if($this->authentification_type == 'oauth2.0_service') {
        $this->client->setClientId($this->client_id_service);
        ...
        $cred = new Google_Auth_AssertionCredentials(
            $this->service_account_name_service,
            array('https://www.googleapis.com/auth/calendar'),
            $this->key_file_content
        );
      ...
      if($this->authentification_type == 'oauth2.0_application') {
        ...
        if($this->client->getAccessToken()){
          if($this->authentification_type == 'oauth2.0_service' && (!strlen($this->client_id_service) || !strlen($this->service_account_name_service) || !strlen($this->key_file_location)))
            return missingServiceAccountDetailsWarning();

          if($this->authentification_type == 'oauth2.0_application' && (!strlen($this->client_id_application) || !strlen($this->service_account_name_application) || !strlen($this->redirect_uri)))
            return missingServiceAccountDetailsWarning();
    ...
    */
    public function add_event($calendar_id, $title, $location, $description, $attendees_user_mail, $date, $time_start, $time_end, $timezone = "Europe/Paris", $optParams = array()) {
      ...
      $inserted_event = $this->service->events->insert($calendar_id, $event, $optParams = array());
    ...
    */
    public function update_event($calendar_id, $event_id, $title, $location, $attendees_user_mail, $date, $time_start, $time_end, $timezone= "Europe/Paris", $optParams = array()) {
      ...
      $attendees = array();
      if($attendees_user_mail) foreach($attendees_user_mail as $user_mail) {
      ...
      $updated_event = $this->service->events->update($calendar_id, $event_id, $event, $optParams = array());
    ...
    */
    public function delete_event($calendar_id, $event_id, $optParams = array()) {
      $this->service->events->delete($calendar_id, $event_id, $optParams = array());
    }
  }